...
; CALLING SEQUENCE:
;      	pLEGEND [,items][,keyword options]
; EXAMPLES:
;	The call:
;		plegend,['Plus sign','Asterisk','Period'],psym=[1,2,3]
;	  produces:
;		-----------------
;		|		|
;		|  + Plus sign	|
...
;	plegend,items,linestyle=lines		; vertical legend---upper left
;	items = ['Plus sign','Asterisk','Period']
;	sym = [1,2,3]
;	plegend,items,psym=sym			; ditto except using symbols
;	plegend,items,psym=sym,/horizontal	; horizontal format
;	plegend,items,psym=sym,box=0		; sans border
;	plegend,items,psym=sym,delimiter='='	; embed '=' betw psym & text
;	plegend,items,psym=sym,margin=2		; 2-character margin
;	plegend,items,psym=sym,position=[x,y]	; upper left in data coords
;	plegend,items,psym=sym,pos=[x,y],/norm	; upper left in normal coords
;	plegend,items,psym=sym,pos=[x,y],/device	; upper left in device coords
;	plegend,items,psym=sym,/position		; interactive position
;	plegend,items,psym=sym,/right		; at upper right
;	plegend,items,psym=sym,/bottom		; at lower left
;	plegend,items,psym=sym,/center		; approximately near center
;	plegend,items,psym=sym,number=2		; plot two symbols, not one
;	plegend,items,/fill,psym=[8,8,8],colors=[10,20,30]; 3 filled squares
; INPUTS:
...
;	    c2_line = [0,2,1]
;	    plegend,c1_items,psym=c1_psym,corners=c1,box=0
...
;	    plegend,['1','22','333','4444'],linestyle=indgen(4),corners=corners
...
;	    plegend,['1','22','333','4444'],linestyle=indgen(4),pos=pos
...
;	Erin Sheldon.  
;		renamed plegend
;		Added bcolor keyword for box color
;		added ability to use c2i color name to color index converter
;		default is no box
;       
;-
pro plegend,help=help,items,linestyle=linestyle,psym=psym,$
           vectorfont=vectorfont $
           ,horizontal=horizontal,vertical=vertical,box=box,margin=margin $
           ,delimiter=delimiter,spacing=spacing,charsize=charsize,$
           pspacing=pspacing $
           ,position=position,number=number,colors=colors,$
...
;
on_error,2
if keyword_set(help) then begin & doc_library,'plegend' & return & endif
;
...
strn = strtrim(n,2)	
IF n_elements(bcolor) EQ 0 THEN bcolor=!p.background ; FOR ERROR MESSAGES
...
symline = (np ne 0) or (nl ne 0)			; FLAG TO PLOT SYM/LINE
 ...
 if nth EQ 0 then thick=replicate(!p.thick,n) else  $
	 if nth EQ 1 then thick = replicate(fix(thick),n) 
...
if n_elements(colors) eq 0 then begin
	...
	if size(colors,/tname) eq 'STRING' then begin
		; requires c2i function which may not exist
...
if n_elements(textcolors) eq 0 then textcolors = !P.color + intarr(n) else $
...
;    if (num eq 1) and vertical then xp = x + xt/2	; IF 1, CENTERED.
            xyouts,xp,yp,vectorfont[i],width=width,color=carr[i] $
              ,size=charsize,align=xalign,/norm
            xt = xt > width
            xp = xp + width/2.
        endif else BEGIN

            if symline and (linestyle[i] ge 0) then plots,xp,yp,color=carr[i] $
...
;        polyfill,[x,xend,xend,x,x],y + [0,0,ywidth,ywidth,0],/norm,color=-1
        polyfill,[x,xend,xend,x,x],y + [0,0,ywidth,ywidth,0],/norm,$
          color=bcolor[0]
