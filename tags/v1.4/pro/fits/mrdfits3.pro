...
;
; CALLING SEQUENCE:
;       Result = MRDFITS3( fileunit,[Extension, Oldextension, Header],
;                       /deja_vu, 
;                       /FSCALE , /DSCALE , /USE_COLNUM, /NO_TDIM,
;                       RANGE=[a,b], Columns=[a,b,...]), error_action=x
;
; INPUTS:
;       Fileunit = Unit of opened file. Position in file will be saved
;             after call.
;
;       Extension= Extension number to be read, 0 for primary array.
;                  Assumed 0 if not specified.
;       Oldextension= the previous extension position.
;
; OUTPUTS:
...
;       DEJA_VU: if set, mrdfits3 reads into the previously defined 
;           struct. Saves much time when mrdfits3 is called repeatedly
;           to read from files that all have the same structure.
...
;       Modified by Phil Fisher from MRDFITS
...
;=============================================================================
function mrd_substruct3, table, column
...
function mrd_getc3, table, column
...
;if not mrd_substruct3(table, column) then begin  
    return, table.(column)  
;endif else begin
...
function mrd_sizcol3, table, column
...
pro mrd_putc3, table, column, value

; This function updates a column in a table.

;if not mrd_substruct3(table, column) then begin   ;Erin Sheldon
   table.(column) = value
;endif else begin
...
function  mrd_dofn3, name, index, use_colnum 
...
function mrd_chkfn3, name, namelist, index 
...
pro mrd_read_ascii3, unit, range, nbytes, nrows, nfld, typarr, posarr,   $
        ...
        if strtrim(nullarr[i]) ne '' then begin
            ...
            if typarr[i] eq 'I' then begin
                if (not keyword_set(old_struct)) then begin
                        ...
                        mrd_putc3, table, i, long(flds)
                endelse
            endif else if typarr[i] eq 'E' or typarr[i] eq 'F' then begin
                if (not keyword_set(old_struct)) then begin
                        table.(i) = float(flds)
                endif else begin
                        mrd_putc3, table, i, float(flds)
                endelse
            endif else if typarr[i] eq 'D' then begin
                if (not keyword_set(old_struct)) then begin
                        table.(i) = double(flds)
                endif else begin
                        mrd_putc3, table, i, double(flds)
                endelse
            endif else if typarr[i] eq 'A' then begin
                if (not keyword_set(old_struct)) then begin
                        table.(i) = flds
                endif else begin
                        mrd_putc3, table, i, flds
                endelse
            endif
        endelse
endfor
...
pro mrd_ascii3, header, structyp, use_colnum,   $
    range, table, $
...
for i=0, nfld-1 do begin
        ...
        fname = mrd_dofn3(fname,i+1, use_colnum)
        fnames[i] = fname

        fname = mrd_chkfn3(fname, fnames, i)
        
        mrd_atype3, fform, ftype, flen
        typarr[i] = ftype
        lenarr[i] = flen
        posarr[i] = fpos
        nullarr[i] = fnull
...
pro  mrd_columns3, table, columns, fnames, fvalues, $
...
if n_elements(tcols) le 0 then begin
        ...
        for i=0, n_elements(tcols)-1 do begin
            if  not keyword_set(old_struct) then begin
                ...
                mrd_putc3, tabx, i, mrd_getc3(table,tcols[i])
            endelse
        endfor
...
pro mrd_read_image3, unit, range, rsize, table 
...
pro mrd_axes_trunc3,naxis, dims, silent
...
pro mrd_image3, header, range, rsize, table, scales, offsets, scaling, $
...
if isgroup GT 0  then begin 
	...
	mrd_axes_trunc3, naxis, dims, keyword_set(silent)
...
pro mrd_scale3, type, scales, offsets, table, header,  $
...
if type ne 0 then begin
        ...
        if ww[0] ne -1 then begin
                for i=0, n_elements(ww)-1 do begin
                    if (not keyword_set(old_struct)) then begin
                        ...
                        mrd_putc3, tabx, ww[i], mrd_getc3(table,ww[i])
                    endelse
                endfor
        endif
        ...
        for i=0, n_elements(w)-1 do begin
                if not keyword_set(old_struct) then begin
                    ...
                    mrd_putc3, tabx, w[i], mrd_getc3(table,w[i])*scales[w[i]] + offsets[w[i]]
                endelse
                istr = strcompress(string(w[i]+1), /remo)
                fxaddpar, header, 'TSCAL'+istr, 1.0, 'Set by MRD_SCALE3'
                fxaddpar, header, 'TZERO'+istr, 0.0, 'Set by MRD_SCALE3'
        endfor
...
; Now process images and random groups.
        ...
        if sz[sz[0]+1] ne 8 then begin
                ...
                endelse
                fxaddpar, header, 'BSCALE', 1.0, 'Set by MRD_SCALE3'
                fxaddpar, header, 'BZERO', 0.0, 'Set by MRD_SCALE3'
                ...
                for i=0, nparam-1 do begin
                        istr = strcompress(string(i+1),/remo)
                        fxaddpar, header, 'PSCAL'+istr, 1.0, 'Added by MRD_SCALE3'
                        fxaddpar, header, 'PZERO'+istr, 0.0, 'Added by MRD_SCALE3'
                        tabx.(0)[i] = table.(0)[i]*scales[i]+offsets[i]
                endfor
                tabx.(1) = table.(1)*scales[nparam] + offsets[nparam]
                fxaddpar, header, 'BSCALE', 1.0, 'Added by MRD_SCALE3'
                fxaddpar, header, 'BZERO', 0.0, 'Added by MRD_SCALE3'
                table = temporary(tabx)
        endelse
endelse

end
pro mrd_read_heap3, unit, header, range, fnames, fvalues, vcls, vtpes, table, $ 
        ...
        if col ge 0 then begin
                ...
                if w[0] eq -1 then begin
                        if  not keyword_set(old_struct) then begin
                            ...
                            mrd_putc3, tablex, i, mrd_getc3(table,col)
                        endelse
                ...
                if not keyword_set(old_struct) then begin
                    ...
                    xx = mrd_getc3(table, ncol)
                    mrd_putc3, tablex, i, reform(xx[0,*])
                endelse
          endelse
endfor 
...
pro mrd_read_table3, unit, range, rsize, structyp, nfld, typarr, table, $
                                      old_struct=old_struct
    ...
    for i=0, nfld-1 do begin 
        ...
        if n_elements(fld) gt 1 then begin
            if not keyword_set(old_struct) then begin
                ...
                mrd_putc3, table, i, fld
            endelse
          endif else begin
            if not keyword_set(old_struct) then begin
                table.(i) = fld[0]
            endif else begin
                mrd_putc3, table, i, fld[0]
            endelse
        endelse
...
pro mrd_tdim3, header, index, flen, arrstr, no_tdim=no_tdim
...
pro mrd_table3, header, structyp, use_colnum,           $ 
...
for i=0, nfld-1 do begin 
        ...
        if dim eq 1 then begin
                ...
                mrd_tdim3, header, i+1, dim, str, no_tdim=no_tdim
                dimfld[i] = str
        endelse
...
;dave johnston addition
  ...
  if num_objects eq 0 then begin
      print,"MRDFITS3:       FILE IS EMPTY"
      return,0
  endif
  ...
  if type eq 0 then begin
      ...
      mrd_image3, header, arange, rsize, table, scales, offsets, status, $
        silent=silent, tempdir=tempdir
      if status ge 0 and rsize gt 0 then mrd_read_image3, unit, arange, rsize, table
      size = rsize
      ...
      mrd_ascii3, header, structyp, use_colnum,   $
      ...
      if status ge 0   and  size gt 0  then begin
                                ...
                                ;*** Read data.
          mrd_read_ascii3, unit,  arange, nbytes, nrows, $
            nfld, typarr, posarr, lenarr, nullarr, table, $
            old_struct=old_struct
          
                                ;*** Extract desired columns.
          if status ge 0 and keyword_set(columns) then $
            mrd_columns3, table, columns, fnames, fvalues, vcls, vtps, $
            scales, offsets, $
      ...
      IF NOT keyword_set(deja_vu) THEN BEGIN 
          ...
          mrd_table3, header, structyp, use_colnum,   $
      ...
      IF status GE 0  AND size GT 0 THEN BEGIN 
          ...
          mrd_read_table3, unit, arange, rsize,  $
          ...
          IF status GE 0 AND keyword_set(columns) THEN BEGIN 
                                ;*** Extract desired columns.
              mrd_columns3, table, columns, fnames, fvalues, $ 
          ...
          IF status GE 0 AND n_elements(vcls) GT 0 THEN BEGIN  
              ...
              mrd_read_heap3, unit, header, arange, fnames, fvalues, $
  ...
  if  status ge 0  and  scaling  and  size gt 0  then begin
      w = where(scales ne 1.d0  or  offsets ne 0.0d0)
                                ...
                                ;*** Apply scalings.
      if w[0] ne -1 then mrd_scale3, type, scales, offsets, table, header,$
