...
IDL_VPTR
applyPixelMask(int argc, IDL_VPTR *argv, char *argk)
{
  ...
  */
  ...
  if (verbose) printf("There are %u masks in %s\n",n_masks,maskFile);
  ...
  if (verbose) printf("Found %d stripes in %s\n",n_stripe,maskFile);
  ...
  if (verbose) printf("Found %u bounding regions...\n",n_bbox);
  ...
  if (verbose) printf("%d masks span %i superpixels...\n",n_masks,n_superpix);
...
void CalculateActualArea()
{
  ...
  for (n=0;n<n_bbox;n++) {
    ...
    gsl_vector_ulong_free(pixnum_array);
    printf("BBOX: %d Area: %lf square degrees\n", n+1,tarea); 
