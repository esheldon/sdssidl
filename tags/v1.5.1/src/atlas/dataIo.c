 ...
 */
      if(om->data != NULL) {
	 base = buff;
	 memcpy(buff, (void *)om->data, om->npix*sizeof(U16));
	 buff += npix*sizeof(U16); 
 ...
 */
	 ...
	 swap2(buff,npix*sizeof(U16));
	 
	 om->data = shRealloc(om->data, npix*sizeof(U16));
	 memcpy((void *)om->data, buff, npix*sizeof(U16));
	 buff += npix*sizeof(U16);
      }
   }
		    ...
		    int len)		/* dimension of buff */
{
   unsigned char *base;			/* saved value of buff at misc times */
   unsigned char *const buff0 = buff;	/* initial value of buff */
   int compress_protocol = RICE_COMPRESSION_V2; /* how was data compressed? */
   int i;
 ...
 */
	 ...
	 memcpy(buff, (void *)ai->pix[i], npix*sizeof(U16));
	 buff += npix*sizeof(U16); 
 ...
 */
      ai->pix[i] = shMalloc(npix*sizeof(U16));
      if(compress_protocol == NOT_COMPRESSED) {
	 swap2(buff, npix*sizeof(U16));
	 memcpy((void *)ai->pix[i], buff, npix*sizeof(U16));
	 buff += npix*sizeof(U16);
      } else {
	 len = (plen == NULL || *plen == 0) ? 0 : (buff0 + *plen) - buff;
