...
;
; PURPOSE:
;     Read all standard FITS data types into arrays or structures. This is a
;     version that is methods so we can access some of the routines outside of
;     mrdfits.
;
; EXPLANATION:
...
;-
PRO MRDFITS::mrd_fxpar, hdr, xten, nfld, nrow, rsize, fnames, fforms, scales, offsets
;
...
function mrdfits::mrd_dofn, name, index, use_colnum, alias=alias
...
; data. 
pro mrdfits::mrd_doff, form, dim, type 
...
function mrdfits::mrd_chkfn, name, namelist, index 
...
function mrdfits::mrd_unsigned_offset, type
...
function mrdfits::mrd_chkunsigned, bitpix, scale, zero, unsigned=unsigned
...
function mrdfits::mrd_unsignedtype, data
...
; Parse the TFORM keyword and return the type and dimension of the
; data.
pro mrdfits::mrd_atype, form, type, slen
...
pro mrdfits::mrd_read_ascii, unit, range, nbytes, nrows, nfld, typarr, posarr,   $
...
pro mrdfits::mrd_ascii, header, structyp, use_colnum,   $
    range, table, $
	...
	if cnt eq 0 then fnull = ''
        ...
        fname = self->mrd_dofn(fname,i+1, use_colnum, alias=alias)
        fnames[i] = fname

        fname = self->mrd_chkfn(fname, fnames, i)
        
        self->mrd_atype, fform, ftype, flen
        typarr[i] = ftype
        lenarr[i] = flen
        posarr[i] = fpos
        nullarr[i] = fnull
...
; user specification.
pro mrdfits::mrd_columns, table, columns, fnames, fvalues, $
...
pro mrdfits::mrd_read_image, unit, range, maxd, rsize, table, rows = rows 
    ...
    type = self->mrd_unsignedtype(table)
    if type gt 0 then begin
	table = table - self->mrd_unsigned_offset(type)
    endif
    return
end 
...
pro mrdfits::mrd_axes_trunc,naxis, dims, silent
...
pro mrdfits::mrd_image, header, range, maxd, rsize, table, scales, offsets, scaling, $
...
; Check if extra NAXISn keywords are present (though this is not legal FITS)
    ...
    xunsigned = self->mrd_chkunsigned(bitpix,  xscale, $
	...
	maxd = gcount
        ...
        self->mrd_axes_trunc, naxis, dims, keyword_set(silent)
	...
	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Reset by MRDFITS v'+self->mrd_version()
	endif
	...
	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Updated by MRDFITS v'+self->mrd_version()
	endif
...
pro mrdfits::mrd_ptrscale, array, scale, offset
...
pro mrdfits::mrd_scale, type, scales, offsets, table, header,  $
	...
	; Just copy the unscaled columns
		...
		sz = size(tabx.(w[i]))
		if sz[sz[0]+1] eq 10 then begin
		    self->mrd_ptrscale, table.(w[i]), scales[w[i]], offsets[w[i]]
		endif
...
pro mrdfits::mrd_varcolumn, vtype, array, heap, off, siz
    ...
    unsigned = self->mrd_unsigned_offset(unsigned)
...
pro mrdfits::mrd_fixcolumn, vtype, array, heap, off, siz
...
; length arrays. 
pro mrdfits::mrd_read_heap, unit, header, range, fnames, fvalues, vcls, vtpes, table, $ 
   ...
   columns=columns, rows = rows, pointer_var=pointer_var, fixed_var=fixed_var
    ...
    for i=0, n_elements(fnames)-1 do begin
		...
		if varying and keyword_set(pointer_var) and vtype ne 'A' then begin
		    self->mrd_varcolumn, vtype, curr_colx, heap, off, siz
		endif else begin
		    self->mrd_fixcolumn, vtype, curr_colx, heap, off, siz
		endelse
...
pro mrdfits::mrd_read_table, unit, range, rsize, structyp, nrows, nfld, typarr, table, rows = rows
...
nxtfld: 
  ...
  for i=0, nfld-1 do BEGIN
      ...
      type = self->mrd_unsignedtype(table.(i))
      
      if type gt 0 then begin
          table.(i) = table.(i) - self->mrd_unsigned_offset(type)
      endif
  endfor
...
pro mrdfits::mrd_tdim, header, index, flen, arrstr, no_tdim=no_tdim
...
pro mrdfits::mrd_table, header, structyp, use_colnum,           $ 
    ...
    self->mrd_fxpar, $
      header, xten, nfld, nrow, rsize, fnames, fforms, scales, offsets
...
;    rsize = fxpar(header, 'NAXIS1') 
    ...
    for i=0, nfld-1 do begin
        ...
        ;; check for a name conflict
        fname = self->mrd_chkfn(fname, fnames2, i)
        ...
        if dim eq 1 then begin
            ...
            self->mrd_tdim, header, i+1, dim, str, no_tdim=no_tdim
            dimfld[i] = str
        endelse
	    ...
	    xunsigned = self->mrd_chkunsigned(bitpix[ppos], scales[i],       $
                                              offsets[i], $
                                              unsigned=unsigned)
        ...
        for j=0, n_elements(types) - 1 do begin
            ...
            if ftype eq types[j] then begin
                ...
                xunsigned = self->mrd_chkunsigned(bitpix[j], scales[i], $
                                                  offsets[i], $
                                                  unsigned=unsigned)

		if xunsigned then begin
		    fxaddpar, header, 'TZERO'+istr, 0, 'Modified by MRDFITS V'+self->mrd_version()
		endif
...
function mrdfits::read, file, extension, header,      $
        structyp = structyp,                    $
    ...
    if keyword_set(version) then begin
        print,'MRDFITS: Version '+self->mrd_version()+' Feb 07, 2005'
    endif
    ...
    if type eq 0 then begin
        ...
        self->mrd_image, header, arange, maxd, rsize, table, scales, offsets, $
          scaling, status, silent=silent, unsigned=unsigned, $
           rows= rows
       if status ge 0 and rsize gt 0 then $
           self->mrd_read_image, unit, arange, maxd, rsize, table, rows = rows
       size = rsize
    endif else if type eq 1 then begin

        ;*** ASCII tables.
        
        self->mrd_ascii, header, structyp, use_colnum,                        $
        ...
        if status ge 0   and  size gt 0  then begin
            ...
            ;*** Read data.
            self->mrd_read_ascii, unit,  arange, nbytes, nrows,   $
              nfld, typarr, posarr, lenarr, nullarr, table,  rows= rows
              
            ;*** Extract desired columns.
            if status ge 0 and keyword_set(columns) then                  $
                self->mrd_columns, table, columns, fnames, fvalues, vcls, vtps, $
        ...
        self->mrd_table, header, structyp, use_colnum,                            $
        ...
        if status ge 0  and  size gt 0  then begin
            ...
            ;*** Read data.
            self->mrd_read_table, unit, arange, rsize,  rows = rows, $
              structyp, nrows, nfld, typarr, table

            if status ge 0 and keyword_set(columns) then begin
        
                ;*** Extract desired columns.
                self->mrd_columns, table, columns, fnames, fvalues,            $
            ...
            if status ge 0 and n_elements(vcls) gt 0 then begin 
                ...
                self->mrd_read_heap, unit, header, arange, fnames, fvalues,       $
    ...
    if  status ge 0  and  scaling  and  size gt 0  then begin
        ...
        ;*** Apply scalings.
        if w[0] ne -1 then self->mrd_scale, type, scales, offsets, table, header,  $
