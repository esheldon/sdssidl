...
;       
; PURPOSE:
;   Add new tags to the structure.  Note this is the same as struct_addfields but
;   it was having namespace collisions with the same program from idlutils
;
; CALLING SEQUENCE:
...
;
;       newstruct = struct_addfields(oldstruct, addstruct, structype=)
;           OR
;       newstruct = struct_addfields(oldstruct, tagnames, values, structype=)
;
; INPUTS: 
...
; EXAMPLE: 
;   ; using a structure
;   newst = struct_addfields(oldstruct, {a:0, b:0d})
;
;   ; using tag names and types
;   tagnames=['ra', 'dec', 'image', 'name']
;   values  =['0d', '0d',  'intarr(1000, 1000)', "'NGC3035'"]
;   newst = struct_addfields(oldstruct, tagnames, values)
;
; CALLED ROUTINES:
;   MRD_STRUCT
;   STRUCT_COMBINE
...
function struct_addfields, struct, tags_or_struct, values, structype=structype
    ...
    if not ok then begin
        print,'Syntax - '
        print,'  newst=struct_addfields(struct, tagnames, values, structype=)'
        print,'    OR'
        print,'  newst=struct_addfields(struct, addstruct, structype=)'
        print,'Use doc_library,"struct_addfields"  for more help.'  
        on_error, 2
        message,'Halting'
    END
