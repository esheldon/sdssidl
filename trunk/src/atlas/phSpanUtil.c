		       ...
		       const REGION *reg) /* the region wherein it dwells */
{
   ...
   U16 *ptr;				/* pointer to allocated space */
   SPAN *sp;				/* == om->s[i] */
#if !defined(NDEBUG)
 ...
 */
   ...
   om->data = shMalloc(om->npix*sizeof(U16));
/*
 ...
 */
   ...
   for(i = 0;i < om->nspan;i++) {
      ...
      memcpy(ptr,&reg->rows[sp->y][sp->x1],npix*sizeof(U16));
		       ...
		       const OBJMASK *om) /* and the OBJMASK */
{
   ...
   U16 *ptr;				/* pointer to OBJMASK's data */
   SPAN *sp;				/* == om->s[i] */
#if !defined(NDEBUG)
   int nrow, ncol;			/* == reg->n{row,col} */
#endif
 ...
 */
   ...
   for(i = 0;i < om->nspan;i++) {
      ...
      npix = sp->x2 - sp->x1 + 1;
      shAssert(sp->y >= 0 && sp->y < nrow && sp->x1 >= 0 && sp->x2 < ncol);
      memcpy(&reg->rows[sp->y][sp->x1],ptr,npix*sizeof(U16));
...
{
   int i,j;
   U16 *row;				/* pointer to row of REGION */
   SPAN *sp;				/* == om->s[i] */
   int nrow, ncol;			/* == reg->n{row,col} */
